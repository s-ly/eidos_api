...
# subscribe_to = bpy.data.scenes["Scene"].my_tool.my_float
# subscribe_to = bpy.context.scene.my_tool.my_float
...
def register():
    ...
    # subscribe_to = bpy.data.scenes["Scene"].my_tool.my_float
    # subscribe_to = bpy.context.scene.my_tool.my_float
    subscribe_to = bpy.context.scene.my_tool
    owner = object()
    notify=msgbus_callback_1
    # notify=bpy.ops.object.modal_operator()
    args=(11, 2, 3, 779005)

    bpy.msgbus.subscribe_rna(key=subscribe_to, owner=owner, args=args, notify=notify)
