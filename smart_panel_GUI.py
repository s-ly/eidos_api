...
class Smart_panel(bpy.types.Panel):
    ...
    bl_idname = 'eidos_smart_panel'  # может быть одновременно несколько панелей
    bl_space_type = 'VIEW_3D'  # Окно расположения    
    bl_region_type = 'UI'   # Панель расположения    
    bl_category = 'EIDOS'      # Группа для панели    
    bl_label = "Smart panel"   # Название нашего меню  
    
    def draw(self, context):
        """Отображающая меню с кнопками"""
        
        
        layout_show_wire = self.layout # блок для элементов управления
        layout_show_wire.label(text="Show wire:") # подписть блока
        split = layout_show_wire.split() # создаёт подблок в блоке layout_show_wire   
        col = split.column(align=True) # создаёт колонку в подблоке split    
        
        # Кнопки в созданной колонке col
        # В кнопки вставляем ссылки на методы созданные в классах
        col.operator(".show_wire_on", text="Wire on")
        col.operator(".show_wire_off", text="Wire off")
        
        
        layout_textured_or_wire = self.layout # блок для элементов управления
        layout_textured_or_wire.label(text="Textured or wire:") # подписть блока
        split = layout_textured_or_wire.split() # создаёт подблок в блоке layout_textured_or_wire
        col = split.column(align=True) # создаёт колонку в подблоке split    

        # Кнопки в созданной колонке col
        # В кнопки вставляем ссылки на методы созданные в классах
        col.operator(".textured_wire_textured", text="Текстура")
        col.operator(".textured_wire_wire", text="Каркас")
